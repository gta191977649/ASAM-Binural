...
class Attention_Binaural(Layer):
    def __init__(self, time_step, spec_dim, embed_dim, mode='dot', init='glorot_uniform',
        ...
        super(Attention_Binaural, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        if self.mode == 'align':
            ...
            self.B_align = self.add_weight(shape=(self.embed_dim, align_hidden), initializer=self.init,
                                           name='{}_SB_align'.format(self.name))
            self.v_align = self.add_weight(shape=(align_hidden, 1), initializer=self.init,
                                           name='{}_v_align'.format(self.name))
        super(Attention_Binaural, self).build(input_shape)  # Be sure to call this somewhere!
    ...
    def call(self, inputs, mask=None):
        ...
        mix_embed_l_left = inputs[0]
        mix_embed_l_right = inputs[1]
        # (None(batch), embed_dim)
        spk_embed_l = inputs[2]
        energy = None
        if self.mode == 'dot':
            # (batch, time, spec_dim, embed_dim) batch_dot(3,1) (batch, embed_dim) = (batch, time, spec_dim)
            energy_L = K.batch_dot(mix_embed_l_left, spk_embed_l, axes=(3, 1))
            energy_R = K.batch_dot(mix_embed_l_right, spk_embed_l, axes=(3, 1))
            energy = (energy_L + energy_R)/2
        elif self.mode == 'align':
            # (batch, time, spec_dim, embed_dim) dot (embed_dim, align_hidden)
            # -> (batch, time, spec_dim, align_hidden)
            hUa = K.dot(mix_embed_l_left, self.U_align)
            hBa = K.dot(mix_embed_l_right, self.B_align)
            # (batch, embed_dim) dot (embed_dim, align_hidden)
            # -> (batch, align_hidden)
            sWa = K.dot(spk_embed_l, self.W_align)
            sWa = sWa.dimshuffle(0, 'x', 'x', 1)
            # -> (batch, time, spec_dim, align_hidden)
            tanh_sWahUa = K.tanh(sWa+hUa+hBa)
