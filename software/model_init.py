...
class ModelInit(object):
    def __init__(self, _log_file):
        ...
        # map spk to idx
        spk_to_idx, idx_to_spk = binaural_data_prepare.get_idx(config.TRAIN_LIST)

        self.train_gen = binaural_data_prepare.get_feature(config.TRAIN_LIST, spk_to_idx=spk_to_idx, min_mix=config.MIN_MIX,
        ...
        self.inp_fea_len, self.inp_fea_dim, self.inp_spec_dim, self.inp_spk_len, self.out_spec_dim = \
            binaural_data_prepare.get_dims(self.train_gen)
